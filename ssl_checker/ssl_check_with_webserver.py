...
#Usage Examples:
# Check login.emory.edu on port 443 every 60 seconds
    #python ssl_check.py login.emory.edu
# Check login.emory.edu every 30 seconds
    #python ssl_check.py login.emory.edu 30
# Check example.com on port 8443 every 60 seconds
    #python ssl_check.py example.com:8443
# Check example.com on port 8443 every 120 seconds
    #python ssl_check.py example.com:8443 120
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: python ssl_check.py <hostname[:port]> [interval_seconds]")
        print("Examples:")
        print("  python ssl_check.py login.emory.edu")
        print("  python ssl_check.py login.emory.edu 30")
        print("  python ssl_check.py example.com:8443")
        print("  python ssl_check.py example.com:8443 60")
        sys.exit(1)

    # Parse hostname and port
    hostname, port = parse_hostname_port(sys.argv[1])

    # Parse interval (default 60 seconds)
    interval = 60
    if len(sys.argv) > 2:
        try:
            ...
            print("Error: Interval must be a valid number")
            sys.exit(1)
    ...
    try:
        ...
        while True:
            ...
            # Create check header
            check_header = f"[Check #{check_count}] [{timestamp}] Checking SSL certificate for {hostname}:{port}..."
            print(check_header)
